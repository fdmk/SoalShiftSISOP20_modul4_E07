...
// Directory yg akan di mount
static  const  char *dirpath = "/home/fdmk/Documents";
static const char *logpath = "/home/fdmk/fs.log";

// deklarasi karakter Caesar cipher
char cs[100] = "9(ku@AW1[Lmvgax6q`5Y2Ry?+sF!^HKQiBXCUSe&0M.b%rI'7d)o4~VfZ*{#:}ETt$3J-zpc]lnh8,GwP_ND|jO9(ku@AW1[L";

void enc1(char *path, int key)
{
...
void dec1(char *path, int key){
  ...
  for (int i = mulai; i <= length; i++)
  {
    ...
    int index = strlen(cs) - 1;
    while(1)
    {
      if(path[i] == cs[index])
      {
        //int index = index - key;
        // while(index < 0) index += strlen(cs);
        // path[i] = cs[index];
        path[i] = cs[index - key];
        break;
      }
...
// Mendapatkan pathfile direktori yang akan di mount
...
// Fungsi2 yg akan diimplementasikan pd fuse
